...
#define ENEMY_TEXT      ("data\\TEXT\\yankiBoss.txt")   //敵のテキストファイル

//================================================================
//静的メンバ変数宣言
//================================================================
CEnemyBoss *CEnemyBoss::m_EnemyBoss = NULL;
...
//コンストラクタ(オーバーロード)
//================================================================
CEnemyBoss::CEnemyBoss(D3DXVECTOR3 pos)
{
	SetPos(&pos);  //位置設定
	SetPosOld(pos);
	m_move = { 0.0f, 0.0f, 0.0f };  //移動量
...
//生成処理
//================================================================
CEnemyBoss *CEnemyBoss::Create(D3DXVECTOR3 pos)
{
	//オブジェクト2Dのポインタ
		...
		if (pEnemy == NULL)
		{
			...
			for (int nCount = 0; nCount < MAX_ENEMY; nCount++)
			{
				if (m_Enemy[nCount] == NULL)
				{
					...
					int nCountBoss = CEnemyManager::GetNumEneBoss();

					nCountBoss++;

					CEnemyManager::SetNumEneBoss(nCountBoss);
...
void CEnemyBoss::Control(void)
{
	...
	if (pCamera->GetType() != CCamera::TYPE_BOSS)
	{
		...
		if (strcmp("SCRIPT", aString) == 0)
		{
			while (strcmp("END_SCRIPT", aString) != 0)
			{
				...
				if (strcmp("MODEL_FILENAME", aString) == 0)
				{
					...
					if (nCntModel == 1 || nCntModel == 2)
					{
						m_nIdxTexture = pTexture->Regist("data\\TEXTURE\\yanki002.jpg");
					}
					else
					{
